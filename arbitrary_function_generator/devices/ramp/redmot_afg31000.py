...
class AFGWorker(object):
    ...
    def ExpRamp(self, dt1, dt2, tau, amp, phase = np.pi, f = 20e3, fs = 500e3):
    ...
    def read_tfw(self, target):
        ...
        with open(target, 'rb') as f:
            ...
            if m[0] != b'TEKAFG3100':
                raise TFW_Read_Error('missing TFW identifier "TEKAFG3100"') # TODO
            if m[1] != 20191229:
                raise TFW_Read_Error('version not 20191229') # TODO
            samples = m[2]
            dac_values = np.fromfile(f, dtype='>u2', count=samples)
        print('{}.tfw readed.'.format(wfm_name))
        return dac_values
    
    def normal_vector(self, vector, ymax=pow(2, 14)-1, ymin=0, dtype='>u2'):
    ...
    def write_tfw(self, wfm_name, dac_values):
        ...
        d = np.array((dac_norm_array & 0x3fff), dtype='>u2') # cast and mask
        samples = len(d)
