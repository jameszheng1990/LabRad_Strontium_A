...
class Andor(object):
    ...
    def __init__(self):
        ...
        if platform.system() == 'Windows':
            os.environ['PATH'] = "C:\\Program Files\\Andor SDK" + ';' + os.environ['PATH']
            if platform.architecture()[0] == '32bit':
                dll = ctypes.windll.LoadLibrary("atmcd32d.dll")
            elif platform.architecture()[0] == '64bit':
                dll = ctypes.windll.LoadLibrary("atmcd64d.dll")
            self.initdir = 'None'
    ...
    def GetReadOutTime(self):
        ...
        Returns:
            (float) valid readout time in seconds.
        """
        readoutTime = ctypes.c_float()
        error = self.dll.GetReadOutTime(ctypes.byref(readoutTime))
        self._log(sys._getframe().f_code.co_name, error)
        return readoutTime.value
    ...
    def GetTemperatureRange(self):
        ...
        error = self.dll.GetTemperatureRange(ctypes.byref(mintemp), ctypes.byref(maxtemp))
        self._log(sys._getframe().f_code.co_name, error)
    ...
    def Initialize(self):
        ...
        self._log(sys._getframe().f_code.co_name, error)
        if error == 20002:
            print('Andor Camera initialized')
        else:
            print('Initialization: {}'.format(ERROR_CODE[error]))
        return self.initdir
    ...
    def ShutDown(self):
        ...
        self._log(sys._getframe().f_code.co_name, error)
        if error == 20002:
            print('Andor Camera shutdown')
        else:
            print('Shutdown: {}'.format(ERROR_CODE[error]))
        return
